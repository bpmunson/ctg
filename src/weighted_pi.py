...
# def weight_by_target_old( eij, fp, w0, probes, targets,
...
#     construct_weights = ansatz_construct_weights(np.array(fpr['rank']), eij, n_probes_per_target=n_probes_per_target)
...
#     construct_weights = pd.SparseDataFrame(construct_weights, columns=columns, index=rows )

#     # make a dataframe out of the pi scores
#     eij = pd.SparseDataFrame(eij,  columns=columns, index=rows )
            
#     # make boolean mask for expressed constructs
#     expressed = w0>0
#     w0 = pd.SparseDataFrame(w0, columns=columns, rows=rows)

#     # calculated weighted pi scores, filtering on only those expressed
#     eij_weighted = eij*construct_weights[expressed]
#     #eij_weighted = eij.multiply(construct_weights.multiply(expressed))
...
screen.run_pi_score_calculation()
