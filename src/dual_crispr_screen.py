...
class Screen(object):
    ...
    def run_construct_fitting(self):
        ...
        # store the name info as separate arrays
        self.probe_a = names['probe_a_id'].values
        self.probe_b = names['probe_b_id'].values
        self.target_a = names['target_a_id'].values
        self.target_b = names['target_b_id'].values
        self.construct_id = names['construct_id'].values

        # get initial weights
        w0 = self._get_initial_weights(allbad)

        # TODO add check for sparity and use regular numpy matrices if dense

        # build into sparse matrices
        self.fc, self.probes = self._make_sparse_matrix(self.probe_a, self.probe_b, fc, return_probes = True)
        self.sdfc = self._make_sparse_matrix(self.probe_a, self.probe_b, sdfc)
        self.pp = self._make_sparse_matrix(self.probe_a, self.probe_b, p_t)
        self.w0 = self._make_sparse_matrix(self.probe_a, self.probe_b, w0)

        # get targets corresponding to probes
        self.targets = self._build_target_array()

        # TESTING 
        # convert into dense
        # self.fc = self.fc.todense()
        # self.sdfc = self.sdfc.todense()
        # self.pp = self.pp.todense()
        # self.w0 = self.w0.todense()
        ...
        if self.options.testing:
            ...
            testing_pp_benchmark = '../data/test_data/output_data/Notebook8Test_pp_0_benchmark.csv'
            path = os.path.join(src, testing_pp_benchmark)
    ...
    def _make_sparse_matrix(self, probe_a, probe_b, feature, return_probes=False):
        ...
        union = np.unique(np.concatenate((self.probe_a, self.probe_b)))
        # enumerate and hash
